...
For each vegetation plot, we further provide information on the dataset it stems from, based on the IDs used in [GIVD](http://www.givd.info). 
...
sPlot Open contains two additional objects. The **'metadata'** matrix contains plot-level metadata, which provide information on the origin of each vegetation plot. 
